...
<script>
  function upload() {
    ...
    xhr.open('POST', 'sign_return.php', true);
    xhr.send(fd);
  };
...
<form name="modalsign" enctype="multipart/form-data" id="modalsign"  method="post">
  ...
  <?php 
    ...
    if(isset($_POST['submit_sign'])) {
      ...
      $upload_dir = "assets/img/sign_return/"; 

      $img = $_POST['hidden_data']; 
      $img = str_replace('data:image/png;base64,', '', $img); 
      $img = str_replace(' ', '+', $img); 
      $data = base64_decode($img); 
      $file2name = "sign-return-" . $booking_id . ".png";
      $file2 = $upload_dir . "sign-return-" . $booking_id . ".png";

      $success = file_put_contents($file2, $data); 

      $sql = "UPDATE checklist SET car_in_sign_image = '$file2name' WHERE booking_trans_id = $booking_id";

      db_update($sql);

      
      echo "<script type='text/javascript'>window.open('returnreceipt.php?booking_id=".$booking_id."');</script>"; 
      ...
      echo "<script>
        window.location.href='mail.php?status=return&booking_id=".$booking_id."';
        </script>
      ";
