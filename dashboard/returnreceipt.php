...
DATE_FORMAT(return_date, '%m/%d/%Y') as return_date,
DATE_FORMAT(return_date_final, '%m/%d/%Y %H:%i:%s') AS return_date_final,
...
$pdf->Write(0, '/'); 
$pdf->SetXY(162, 26.5); 
$pdf->Write(0, date('d/m/Y', strtotime($return_date))); 
$pdf->SetXY(173, 26.5); 
$pdf->Write(0, '@'); 
$pdf->SetXY(176, 26.5); 
...
{
    $pdf->Image('assets/img/sign_pickup/'.$car_out_sign_image, 75, 124.5, 27); 
}

if($car_in_sign_image == '' || $car_in_sign_image == null)
{
    
}
else
{
    $pdf->Image('assets/img/sign_return/'.$car_in_sign_image, 175, 124.5, 27); 
    $pdf->Image('assets/img/sign_return/'.$car_in_sign_image, 165, 271, 27); 
}
...
// 		$pdf->Write(0, $fullname); 
$pdf->SetXY(140, 209); 
$pdf->Write(0, date('d/m/Y', strtotime($return_date_final))); 
$pdf->SetXY(160, 209); 
$pdf->Write(0, '@'); 
$pdf->SetXY(165, 209); 
$pdf->Write(0, date('H:i',strtotime($return_date_final))); 
