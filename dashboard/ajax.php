...
func_setReqVar();
if ($t == 'selectNRIC') { 
	$sql = "SELECT firstname, address, postcode, city, country, title, lastname, age, phone_no, email, ref_name, ref_phoneno, license_no, ref_relationship, ref_address, license_exp FROM customer WHERE nric_no='" . $_GET['nric'] . "'"; 
	db_select($sql);
	...
	if (db_rowcount() > 0) { 
		...
		// $license_exp = db_get(0, 15); 
	} 
	echo $firstname . "|||" . $address . "|||" . $postcode . "|||" . $city . "|||" . $country . "|||" . $title . "|||" . $lastname . "|||" . $age . "|||" . $phone_no . "|||" . $email . "|||" . $ref_name . "|||" . $ref_phoneno . "|||" . $license_no . "|||" . $ref_relationship . "|||" . $ref_address . "|||" . $license_exp; 
} 
else if($t=='calcPrice'){ 
	$total = 0; 
	$estimate_total = 0; 
